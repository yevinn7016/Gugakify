...
Future<void> _startPolling() async {
  ...
  _timer = Timer.periodic(const Duration(seconds: 3), (_) async {
    try {
      final projectId = project.aiJobId;
      if (projectId == null) return;

      final uri = Uri.parse(
        '${ApiConfig.baseUrl}/projects/$projectId/status',
      );
      ...
      if (response.statusCode == 200) {
        ...
        final total = data['total_steps'] as int? ?? 4;
        final newProgress = step / total;
  ...
  Widget build(BuildContext context) {
    ...
    return GugakifyAppScaffold(
      ...
      child: Stack(
        children: [
          const Positioned.fill(child: _ProcessingAtmosphere()),
          Column(
            ...
            children: [
              ...
              _ProcessingCard(progress: progress, currentStep: _steps[index]),
...
class _ProcessingAtmosphere extends StatelessWidget {
  const _ProcessingAtmosphere();
...
class _ProcessingHeader extends StatelessWidget {
  const _ProcessingHeader({required this.projectName, required this.onBack});
...
class _ProcessingCard extends StatelessWidget {
  const _ProcessingCard({required this.progress, required this.currentStep});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        children: [
          const Positioned.fill(child: _ProcessingInkWash()),
          Column(
            children: [
              const Align(
                alignment: Alignment.centerLeft,
                child: _StepPill(label: 'AI Gugak Conversion'),
              ),
              const SizedBox(height: 22),
...
class _ProcessingInkWash extends StatelessWidget {
  const _ProcessingInkWash();
...
class _ProcessingInkPainter extends CustomPainter {
  @override
  ...
  @override
  bool shouldRepaint(covariant _ProcessingInkPainter oldDelegate) => false;
}
